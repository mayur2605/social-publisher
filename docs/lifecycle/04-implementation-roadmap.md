...
| M4: Staging             | Railway services, HTTPS, backups, restore, rollback, monitoring     | Config prepared; execution pending               | Phase 6 gate signed with deployment and recovery evidence       |
...
| Legal/setup and deployment configuration | FR-12, NFR-06       | legal pages, Dockerfiles, Railway TOML, existing runbooks |
...
| E-04 | P1       | Large-file throughput and memory                  | Engineering; real Drive and destinations | 2 GiB source measured within provider limits; bounded memory; stalled transfers recover; record any optimization             |
...
The product owner must provide or authorize access to Google, Meta, TikTok, Stripe test, and Railway accounts. Configure secrets in local `.env.local` or Railway variables; never put credentials in planning documents, tickets, commits, or chat. `npm run setup:check` reports missing names without revealing values.
...
| PostgreSQL + pg-boss + separate Railway worker      | Agreed   | Durable browser-independent scheduling                                         |
